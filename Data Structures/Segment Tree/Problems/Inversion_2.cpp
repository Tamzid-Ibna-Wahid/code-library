...
struct segtree{
  ...
  item number_of_one_on_right(int v, int x, int lx, int rx){
    if(rx - lx == 1) return lx;
    int m = (lx + rx)/2;
    int res;
    int left_part = 2 * x + 1;
    int right_part = left_part + 1;
    if(values[right_part] > v){
      res = number_of_one_on_right(v, right_part, m, rx);
    }
    else{
       res = number_of_one_on_right(v - values[right_part], left_part, lx, m);      
    }
    return res;
  }
  
  item number_of_one_on_right(int v){
    return number_of_one_on_right(v, 0, 0, size);
  }
...
void siuuuuu(){
      ...
      vector<int>ans;
       ...
       for(int i = n-1;i>=0;i--){
          int num = st.number_of_one_on_right(v[i]);       
        st.set(num, 0);
          ans.pb(num+1);
       }
