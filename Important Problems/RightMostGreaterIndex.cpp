...
// right most index which value is greater then the current element
vector<int> rightMostIndxGreater(vector<int> &v){
    int n = sz(v);
    vector<int> indx(n, -1);
    vint st, st2;       
    for (int i=n-1;i>= 0;i--) {
        if (st.empty() ||v[st.back()]<v[i]) {
            ...
            auto it=lower_bound(all(st2), v[i]);
            int pos = it-st2.begin();
            indx[i] =st[pos];   
        }
    }
}
