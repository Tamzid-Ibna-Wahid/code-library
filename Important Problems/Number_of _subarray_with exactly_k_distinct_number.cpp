...
// count the number of subarray such that length between l to r and distinct element is exactly k


int atMostK(vector<int> &arr, int k, int L, int R) {
    int n = arr.size();
    int res = 0;
...
int exactlyK(vector<int> &arr, int k, int L, int R) {
    return atMostK(arr, k, L, R) - atMostK(arr, k - 1, L, R);
}
...
void siuuuuu(){
       ...
       cout<<exactlyK(v, k, l, r)<<endl;  // subarray size should be l to r
