...
void siuuuuu(){
        ...
        vector<string>ans;
         ...
         while(!v.empty()){
             ...
             string s = v.back().first;
            if(s=="ask"){
                if(d.findpar(x)==d.findpar(y))ans.pb("YES");
                else ans.pb("NO");
            }
            else{
               d.unionSize(x,y);
         ...
         reverse(all(ans));
         REPn(i,sz(ans))cout<<ans[i]<<endl;
         
/* 
In forward direction, "cut u v" = remove edge (u, v).
In reverse direction, "cut u v" = add edge (u, v) back.
So here "cut" is not really cutting — because we’re going backwards, it becomes an edge addition.
*/
